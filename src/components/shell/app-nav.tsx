...
export function sidebarNavItemClassName(
  ...
  return cn(
    "flex items-center gap-2.5 rounded-sm px-2 text-sm focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-fg",
    mobile ? "min-h-11 py-2" : "h-9 min-h-9",
    controlMotion,
    "[&_svg]:size-[var(--nav-icon-size)] [&_svg]:shrink-0",
    active
      ? "bg-selected-bg font-medium text-fg [&_svg]:text-[var(--brand-accent)]"
...
export function SidebarSettingsLink() {
  ...
  return (
    <NavLink item={appSettingsNav} pathname={pathname} mobile={false} />
  );
}
...
export function AppNav({
  ...
  return (
    ...
    >
      <div className="flex min-h-0 flex-1 flex-col gap-px overflow-y-auto overflow-x-hidden overscroll-contain px-2 pt-3 pb-2">
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        nested &&
          (mobile ? "h-auto min-h-11 items-start py-2.5" : "h-auto min-h-9 py-2"),
      )}
    >
      <Icon aria-hidden />
      <span
        className={cn(
          "min-w-0",
          nested ? "text-left leading-snug" : "truncate",
        )}
      >
        {item.label}
      </span>
    ...
    <div className="flex flex-col gap-px">
      ...
      <Collapse open={open}>
        <div
          ...
          className="flex flex-col gap-px pb-0.5 pl-[1.625rem]"
        >

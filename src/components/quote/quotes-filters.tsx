...
export function QuotesFilters({
  ...
  function navigate(next: Partial<QuotesFilterValues>) {
    replace(
      buildQuotesHref({
  ...
  // Alleen klanten met offertes; naam en telling komen uit het facet.
  ...
  const chips = [
    values.status
      ? {
          key: "status",
          label: "Status",
          value: quoteStatusLabels[values.status as keyof typeof quoteStatusLabels] ?? values.status,

...
import { parseQuoteForm, parseQuoteStatusForm } from "@/lib/quote-validation";
...
function revalidateQuotePaths(quoteId?: string) {
  ...
  if (quoteId) revalidatePath(`/offertes/${quoteId}`);
}
...
export async function updateQuoteAction(
  ...
  try {
    ...
    const input = parseQuoteForm(formData);
    await updateQuote(id, input, session.user.id);
...
export async function updateQuoteStatusAction(
  ...
  try {
    ...
    const id = String(formData.get("id") ?? "");
    const status = parseQuoteStatusForm(formData);
